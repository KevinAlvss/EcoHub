...
export class UserService {
    ...
    async getUserById (userId: string): Promise<GetUser> {
        ...
        const response = await api.get<GetUser>(`api/v1/usuario/Usuario?userId=${userId}`)
        return response.data;
    }
    ...
    async updateExistingUser (user: AddOrUpdateUserRequest, userId: string): Promise<any> {

        const response = await api.put(`api/v1/usuario/Usuario?userId=${userId}`,user);
        return response.data
    }

    async deleteExistingUser (userId: string): Promise<any> {

        const response = await api.delete(`api/v1/usuario/Usuario?userId=${userId}`);
        return response.data;
    }
 }